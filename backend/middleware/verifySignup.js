...
checkDuplicateEmailOrMobileNumber = (req, res, next) => {
    ...
    User.findOne({$or: [
        ...
        if(user){
            next(new ErrorResponse("Failed! Mobile number or Email is already in use!", 401))
        }
         next();
    });
};
...
sendOTPToMobileNumber = (req, res) => {
    client
          ...
          .then((data) => {
              console.log(data)
              res.status(200).send(`OTP sent to ${req.body.mobile_number} and is valid for only 10 minutes`)
          })
          .catch(err => {
              console.log(err)
              next(new ErrorResponse(err, 500))
